...
the project must have a  <code>lib/*.rb</code> file. In addition
is should have a <code>VERSION</code> file, optimally with a
<code>name</code> entry. As a fallback, for the name and version,
if for some reason you can not use the VERSION file accroding
to the POM specification, you can place the entries in
<code>.meta/name</code> and <code>.meta/version</code>.
That is the bare minimum for a project to to be loadable via
Rolls.

Optionally you may provide a <code>.meta/loadpath</code> file if
the loadpath is not the default <code>lib/</code>. However
<code>lib</code> is always expected to be one of your loadpath.

See POM[http://proutils.github.com/pom] for more information.
