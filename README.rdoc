...
I suspect you will be using RubyGems too, in which case, depending on the
precedence you want, do:
...
the project must contain a +VERSION+ file with a +name+ and +version+
entry. That is the bare minimum for a project to to be loadable via
Rolls. Optionally you may provide a +path+ entry if the loadpath 
is not the +lib+ default, as well as a release +date+ entry.
