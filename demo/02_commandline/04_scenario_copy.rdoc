...
As a developer I want to be able to setup new library environments
based on existing environments.

To copy the current environment, I should be able to issue the command.
...
get an error telling us that we need to use the `--force` option if we wish
to overwrite it.
...
By supplying the force option we can go ahead and force the operation.
