...
The example projects are stored in `tmp/projects`. To add a lookup location
...
To confirm that our environment has indeed been setup as expected we can read
in the environment configuration file and ensure it contains the projects,
which are TryMe v1.0 and TryMe v1.1.

  text = File.read('tmp/config/environments/production')
